...
CNAKO3_CMD=./cnako3_container.sh

run_test_utils() {
  $CNAKO3_CMD test/test_utils.sh
}

run_test_json() {
  local infile="$1"; shift

  cat $infile | $CNAKO3_CMD test/test_json.nako3
}

run_lex() {
  local infile="$1"; shift

  cat $infile | $CNAKO3_CMD lexer.nako3
}

run_parse() {
  local infile="$1"; shift

  cat $infile | $CNAKO3_CMD parser.nako3
}

run_codegen() {
  local infile="$1"; shift

  cat $infile | $CNAKO3_CMD codegen.nako3
}
